...
from i75.graphic_primitives import circle
...
class Ball:
    def __init__(self,
                 ...
                 size: int,
                 motion: Vector) -> None:
        self.colour = Colour.fromrgb(*from_hsv(random.random(),
                                               1.0,
                                               1.0))
        self.centre = centre
        self.size = size
    ...
    def render(self, screen: FullColourScreen, colour: Optional[picographics.Pen] = None):
        circle(screen,
               round(self.centre.x),
               round(self.centre.y),
               self.size,
               colour if colour is not None else self.colour)
...
def generate_ball(size: int, v: Vector) -> Ball:
    return Ball(v, size, Vector(random.random() * 8,
                                random.random() * 8))
...
class BouncingBalls:
    def __init__(self, i75: I75, manager: ScreenManager) -> None:
        ...
        self.screen = FullColourScreen(64, 64)

        manager.set_screen(self.screen)

        self.balls = [generate_ball(1, Vector(10, 20)),
                      generate_ball(3, Vector(20, 10)),
                      generate_ball(5, Vector(40, 10))]
    ...
    def render(self, i75: I75, frame_time: int) -> bool:
        ...
        while self.frame_time > self.fixed_update_time:
            for ball in self.balls:
                ball.render(self.screen, self.black)
        ...
        while self.new_balls_time > 5000:
            [b.render(self.screen, self.black) for b in self.balls]
            ...
            [b.render(self.screen) for b in self.balls]
